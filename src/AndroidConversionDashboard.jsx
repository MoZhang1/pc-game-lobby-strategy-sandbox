...
function ReviewPage() {
  ...
  const nextSteps = experimentId === 'local-hot-v2'
    ...
    : experimentId === 'local-hot-observation'
      ? [{ title: '实验 1｜回退游戏位：撤掉新增棋牌游戏位，恢复被减少的营收游戏位。', detail: '连续观察 7 天，按市新增排序规则保持不变；以当前 13.16% 本地热门点击为直接对照，并看是否向 7/08–7/14 的 15.14% 回升。' }, { title: '实验 2｜在回退版本上验证棋牌需求：不减少营收位，仅将棋牌游戏作为本地热门末位新增展示。', detail: '连续观察 7 天；对照组为实验 1 的回退版本，实验组只多 1 个末位棋牌游戏。用于区分“棋牌本身不受欢迎”与“替换营收位造成点击下降”。' }, { title: '实验 3｜按城市分层放量：只在实验 2 点击提升的城市保留末位棋牌游戏。', detail: '其余城市维持实验 1 回退版本。以本地热门点击为主指标、总启动不下降为护栏；连续两周稳定后再全量固化。' }]
