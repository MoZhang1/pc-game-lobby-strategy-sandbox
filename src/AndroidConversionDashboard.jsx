...
function LocalPackageConfigExperiment() {
  return <section className="pageSection"><div className="sectionTitle"><div><h2>本地包运营配置实验</h2><p>已完成首个 7 天观察周期</p></div></div><Card className="experimentBrief geoExperimentPlan"><div><span>实验地区</span><b>山东、广东、安徽</b><small>三个实验地区的本地包运营配置表于 8/26 开始配置。</small></div><div><span>对比区间</span><b>8/19–8/25 vs 8/26–9/1</b><small>已收齐配置后 7 天数据。</small></div><div><span>复盘指标</span><b>页面曝光→配置推荐点击</b><small>同步观察页面曝光→算法推荐点击；两类点击不相加。</small></div></Card></section>;
}
...
function LocalPackageGeoReview() {
  ...
  return <section className="localPackageGeoReview">
    <section className="pageSection"><div className="sectionTitle"><div><h2>本地包地区分发总览</h2><p>{formatDate(meta.start)}–{formatDate(meta.end)} · 按新增用户口径汇总</p></div></div>
      ...
      <div className="reviewCallout geoConclusion"><b>省级结论</b><p>{selectedPeriod.id === '2026-08-26_2026-09-01' ? <>本轮为配置实验复盘：全量<strong>算法推荐点击率 {rate(total.algorithmClicks, total.pageExposure)}</strong>、<strong>配置推荐点击率 {rate(total.configuredClicks, total.pageExposure)}</strong>。山东配置推荐点击较对照期<strong> -17.8pp</strong>，优先复盘；广东<strong> +5.0pp</strong>、安徽<strong> +2.6pp</strong>但样本较小，继续观察，当前不扩大范围。</> : <>本页仅按页面曝光后的两类推荐点击判断：全量<strong>算法推荐点击率 {rate(total.algorithmClicks, total.pageExposure)}</strong>、<strong>配置推荐点击率 {rate(total.configuredClicks, total.pageExposure)}</strong>。在新增≥100的省份中，<strong>{topProvinces.join('、') || '暂无'}</strong>两类点击均较好，可保留规则并提炼城市样本。<strong>{priorityProvinces.join('、') || '暂无'}</strong>需优先优化：算法点击弱先调候选池/城市权重，配置点击弱先调配置游戏和排序。</>}</p></div>
    </section>
