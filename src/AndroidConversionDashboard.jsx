...
const PC_ACTIVE_EXPERIMENTS = [
  { id: 'pc-september15', title: '完整观察周 · 上周计划待核实生效', verdict: '整体持平，暂不支持扩大联运数量', start: '2026-09-09', end: '2026-09-15', beforeStart: '2026-09-02', beforeEnd: '2026-09-08', baseline: '9/2–9/8', baselineRate: 24.2329016438, baselineHelper: '149,525 / 617,033', resultRate: 24.2625025079, resultHelper: '149,951 / 618,036', delta: 0.0296008641, content: '上周计划提高本地热门营收游戏权重并增加联运数量；源表未提供配置生效记录，本轮只评价整体走势。', conclusion: '进入营收游戏占比 24.23%→24.26%（+0.03pp），基本持平。营收棋牌 24.101%→24.134%，联运创角 0.181%→0.174%（1,115→1,078 UV）；联运未体现持续改善。用户池按日合计 +0.16%，进入 UV +0.28%。先核对上周计划是否实际生效，再做单项排序对照。', next: NEXT_PC_ACTIVE_PLAN },
...
function ActiveExperimentTrendChart({ items, markerDate, ariaLabel = '安卓活跃用户本地热门实验周期趋势' }) {
  const [hoveredIndex, setHoveredIndex] = useState(null);
...
function PcActiveExperimentReview() {
  ...
  return <section className="pageSection"><div className="sectionTitle"><div><h2>活跃用户定向分发实验复盘</h2><p>PC 活跃用户 · 每个实验区间统一展示前后两周数据</p></div></div>
    ...
    <section className="experimentBlock"><Card className="experimentBrief activeUserExperiment"><div><span>实验周期</span><b>{experiment.title} · {formatDate(experiment.start)}–{formatDate(experiment.end)}</b><small>实验前对比：{experiment.baseline}</small></div><div><span>实验模块</span><b>精品游戏分类配置 / 高活跃地区本地热门</b></div><div><span>实验内容</span><b>{experiment.content}</b></div></Card>
    <div className="reviewMetrics activeUserReviewMetrics"><Metric label="实验前基线" value={`${experiment.baselineRate.toFixed(2)}%`} helper={`${experiment.baseline} · ${experiment.baselineHelper}`} icon={BarChart3} /><Metric label="实验期结果" value={`${experiment.resultRate.toFixed(2)}%`} helper={`${formatDate(experiment.start)}–${formatDate(experiment.end)} · ${experiment.resultHelper}`} tone={tone} icon={experiment.delta >= 0 ? TrendingUp : TrendingDown} /><Metric label="实验变化" value={formatPp(experiment.delta)} helper="变现率（进入营收游戏）" tone={tone} icon={experiment.delta >= 0 ? TrendingUp : TrendingDown} /></div>
    ...
    <section className="pageSection"><div className="sectionTitle"><div><h2>实验前后趋势</h2><p>{formatDate(experiment.beforeStart)}–{formatDate(experiment.end)} · 虚线为实际配置日或观察周分界</p></div></div><Card className="chartCard"><ActiveExperimentTrendChart items={comparisonItems} markerDate={experiment.markerDate || experiment.start} ariaLabel="PC 活跃用户定向分发实验周期趋势" /></Card></section>
...
function TargetGamePage({ platform }) {
  ...
  const conclusion = isPc ? '9/1–9/15 进入营收游戏占比 24.23%，与 8 月 24.22% 基本持平。最新完整周 24.23%→24.26%（+0.03pp）；联运创角 0.181%→0.174%，建议先确认上周配置生效情况，再做营收游戏排序对照，本周动作待定。' : '9/1–9/15 进入营收游戏占比 42.90%，较 8 月 42.44% 高 0.46pp。最新完整周 42.71%→43.13%（+0.42pp），建议保留当前配置、观察稳定性，本周新增实验待定。缺少 Banner 点击和对照组，不能单独归因为素材。';
  return <>
