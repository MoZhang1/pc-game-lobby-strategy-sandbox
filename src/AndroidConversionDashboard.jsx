...
function GlobalEventTable() {
  const all = getSummary('2026-06-20', '2026-09-01');
  const packagePeriod = getSummary('2026-07-15', '2026-09-01');
...
function GlobalDataPage({ embedded = false }) {
  const [range, setRange] = useState({ start: '2026-08-02', end: '2026-09-01' });
  ...
  return <>
    {!embedded && <header className="pageIntro"><div><h1>安卓新用户分发数据</h1><p>总启动与模块点击总览</p></div><span>数据更新至 2026/09/01</span></header>}
    <Card className="filterCard"><div className="filterCopy"><CalendarDays /><div><b>数据日期</b><span>模块数据有效起始 6/20；7/15 起本地包点击单列展示</span></div></div><DateRangeInput label="数据日期" isLabelHidden value={range} onChange={value => value && setRange(value)} min="2026-06-20" max="2026-09-01" numberOfMonths={1} /><Button label="应用筛选" variant="primary" onClick={() => setAppliedRange(range)} /></Card>
...
function AndroidActiveExperimentReview() {
  const [experimentId, setExperimentId] = useState('active-latest-observation');
...
function PcActiveExperimentReview() {
  const [experimentId, setExperimentId] = useState('pc-target-distribution-v1');
  ...
  return <section className="pageSection"><div className="sectionTitle"><div><h2>活跃用户定向分发实验复盘</h2><p>PC 活跃用户 · 每个实验区间统一展示前后两周数据</p></div></div>
    ...
    <div className="reviewMetrics activeUserReviewMetrics"><Metric label="实验前基线" value={`${experiment.baselineRate.toFixed(2)}%`} helper={`${experiment.baseline} · ${experiment.baselineHelper}`} icon={BarChart3} /><Metric label="实验期结果" value={`${experiment.resultRate.toFixed(2)}%`} helper={`${formatDate(experiment.start)}–${formatDate(experiment.end)} · ${experiment.resultHelper}`} tone={tone} icon={experiment.delta >= 0 ? TrendingUp : TrendingDown} /><Metric label="实验变化" value={formatPp(experiment.delta)} helper="变现率（进入营收游戏）" tone={tone} icon={experiment.delta >= 0 ? TrendingUp : TrendingDown} /></div>
    <div className="reviewCallout activeUserCallout"><b>本轮结论：{experiment.delta >= 0 ? '正向' : '暂无明确效果'}</b><p>{experiment.conclusion}</p></div>
...
function TargetGamePage({ platform }) {
  ...
  return <>
    <header className="pageIntro"><div><h1>{name}分发数据</h1><p>活跃用户进入营收游戏的月度表现</p></div><span>数据更新至 2026/09/01</span></header>
...
function ReviewPage({ experimentIds = EXPERIMENTS.map(item => item.id), title = '实验复盘', subtitle = '按实验区间切换查看 · 7/15 起本地包点击单列展示', showPackageWeeks = false, embedded = false }) {
  ...
  if (experiment.isOngoing) return <>
    {!embedded && <header className="pageIntro"><div><h1>{title}</h1><p>{subtitle}</p></div><span>数据更新至 2026/09/01</span></header>}
...
function AndroidNewUserPage() {
  ...
  return <><header className="pageIntro"><div><h1>安卓新用户分发数据</h1><p>纯新增用户的总启动、模块点击与实验复盘</p></div><span>全局数据至 2026/09/01 · 本地包分城市数据支持 8/17 起滚动观察</span></header>
