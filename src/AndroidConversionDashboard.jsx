...
function GlobalDataPage({ embedded = false }) {
  ...
  return <>
    {!embedded && <header className="pageIntro"><div><h1>安卓新用户分发数据</h1><p>总启动与模块点击总览</p></div><span>数据更新至 2026/08/18</span></header>}
...
function AndroidActiveExperimentReview() {
  const [experimentId, setExperimentId] = useState('active-local-hot-v2');
...
function LocalPackageGeoReview() {
  ...
  const benchmark = { reach: numericRate(total.pageExposure, total.newUsers), page: numericRate(total.totalClicks, total.pageExposure), e2e: numericRate(total.totalClicks, total.newUsers) };
  const insightFor = item => {
    ...
    const reach = numericRate(item.pageExposure, item.newUsers);
    const page = numericRate(item.totalClicks, item.pageExposure);
    const e2e = numericRate(item.totalClicks, item.newUsers);
    if (e2e >= benchmark.e2e + 4) return { group: '表现较好', issue: `${reach >= benchmark.reach ? '入口覆盖' : '入口覆盖仍可补'}${page >= benchmark.page ? '、页面承接均较好' : '较稳'}`, action: '保持当前规则；按城市拆看高转化来源，作为同类城市实验候选。' };
    if (reach < benchmark.reach - 4 && page < benchmark.page - 4) return { group: '优先优化', issue: '入口覆盖和页面承接均偏低', action: '先排查本地包曝光/版本覆盖，再按城市重排配置与算法推荐位。' };
    if (reach < benchmark.reach - 4) return { group: '待优化', issue: '新增→页面曝光偏低，主要是入口覆盖不足', action: '优先核查本地包展示、地区归属与版本覆盖；页面内容暂不先动。' };
    if (page < benchmark.page - 4) return { group: '待优化', issue: '页面曝光→总点击偏低，主要是页面承接不足', action: '优先按城市调整配置/算法推荐游戏与排序，再观察点击。' };
    return { group: '稳定观察', issue: '整体接近全量均值', action: '维持当前规则；优先在高新增城市做小流量对照实验。' };
  };
  ...
  return <section className="localPackageGeoReview">
    <section className="pageSection"><div className="sectionTitle"><div><h2>本地包地区分发总览</h2><p>{formatDate(meta.start)}–{formatDate(meta.end)} · 按新增用户口径汇总</p></div></div>
      <Card className="geoToolbar"><div><span>数据时间区间</span><b>{formatDate(meta.start)}–{formatDate(meta.end)}（{meta.days} 天）</b><small>本地包上线以来已收数的完整日数据</small></div><label className="eventSelect">省份结论筛选<select value={provinceScope} onChange={event => setProvinceScope(event.target.value)} aria-label="选择省份结论筛选"><option value="全部">全部省份</option><option value="表现较好">表现较好</option><option value="稳定观察">稳定观察</option><option value="待优化">待优化</option><option value="优先优化">优先优化</option><option value="低样本观察">低样本观察</option><option value="无数据">无数据</option></select></label></Card>
      ...
      <div className="reviewCallout geoConclusion"><b>省级结论</b><p>在新增≥100的省份中，<strong>{topProvinces.join('、') || '暂无'}</strong>表现较好：新增→总点击显著高于全量 {rate(total.totalClicks, total.newUsers)}，可保留现规则并提炼城市样本。<strong>{priorityProvinces.join('、') || '暂无'}</strong>需优先优化：其中入口覆盖偏低的省份先查展示/版本覆盖，页面承接偏低的省份先调城市推荐和排序。其余省份先稳定观察。</p></div>
    </section>
    <section className="pageSection"><div className="sectionTitle"><div><h2>省级分发转化与建议</h2><p>全量省份均保留；判断基于本期累计数据，低样本不参与优劣排序</p></div></div><Card className="geoTableCard"><div className="geoTable geoInsightTable" role="table" aria-label="本地包省级分发转化与建议"><div className="geoTableHead" role="row"><span>省份</span><span>新增UV</span><span>新增→页面曝光</span><span>页面曝光→总点击</span><span>新增→总点击</span><span>判断 / 主要差异</span><span>建议动作</span></div>{provinceRows.map(item => { const insight = insightFor(item); return <div className="geoTableRow" role="row" key={item.province}><strong>{item.province}</strong><span data-label="新增UV">{item.newUsers ? item.newUsers.toLocaleString() : '—'}</span><span data-label="新增→页面曝光">{rate(item.pageExposure, item.newUsers)}</span><span data-label="页面曝光→总点击">{rate(item.totalClicks, item.pageExposure)}</span><b data-label="新增→总点击">{rate(item.totalClicks, item.newUsers)}</b><span data-label="判断 / 主要差异"><em className={`geoTag ${insight.group === '表现较好' ? 'good' : insight.group === '优先优化' || insight.group === '待优化' ? 'bad' : ''}`}>{insight.group}</em><small>{insight.issue}</small></span><span data-label="建议动作" className="geoAction">{insight.action}</span></div>})}</div></Card></section>
...
function ReviewPage({ experimentIds = EXPERIMENTS.map(item => item.id), title = '实验复盘', subtitle = '按实验区间切换查看 · 7/15 起本地包点击单列展示', showPackageWeeks = false, embedded = false }) {
  ...
  return <>
    {!embedded && <header className="pageIntro"><div><h1>{title}</h1><p>{subtitle}</p></div><span>数据更新至 2026/08/18</span></header>}
...
function AndroidNewUserPage() {
  ...
  return <><header className="pageIntro"><div><h1>安卓新用户分发数据</h1><p>纯新增用户的总启动、模块点击与实验复盘</p></div><span>全局数据至 2026/08/18 · 本地包地区数据至 2026/08/24</span></header>
    <div className="activeDistributionTabs" role="tablist" aria-label="安卓新用户分发页签"><button role="tab" aria-selected={tab === 'global'} className={tab === 'global' ? 'selected' : ''} onClick={() => setTab('global')}>全局数据</button><button role="tab" aria-selected={tab === 'local-hot'} className={tab === 'local-hot' ? 'selected' : ''} onClick={() => setTab('local-hot')}>本地热门实验复盘</button><button role="tab" aria-selected={tab === 'local-package-geo'} className={tab === 'local-package-geo' ? 'selected' : ''} onClick={() => setTab('local-package-geo')}>本地包分城市数据</button><button role="tab" aria-selected={tab === 'local-package-review'} className={tab === 'local-package-review' ? 'selected' : ''} onClick={() => setTab('local-package-review')}>本地包实验复盘</button></div>
    {tab === 'global' ? <GlobalDataPage embedded /> : tab === 'local-hot' ? <ReviewPage key="new-local-hot-review" embedded experimentIds={LOCAL_HOT_EXPERIMENT_IDS} title="本地热门实验复盘" subtitle="安卓新用户 · 按实验区间切换查看" /> : tab === 'local-package-geo' ? <LocalPackageGeoReview /> : <><ReviewPage key="new-local-package-review" embedded experimentIds={LOCAL_PACKAGE_EXPERIMENT_IDS} title="本地包实验复盘" subtitle="安卓新用户 · 本地包正式上线后的变化持续在此展示" showPackageWeeks /><LocalPackageExperimentProtocol /></>}
  </>;
}
