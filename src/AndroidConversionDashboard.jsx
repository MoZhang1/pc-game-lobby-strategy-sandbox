...
const EXPERIMENTS = [
  ...
  {
    id: 'total-start-dip-sep4', title: '异常诊断 · 9/4 总启动低点', module: '总启动 / 游戏模块开始玩',
...
function PcActiveExperimentReview() {
  ...
  return <section className="pageSection"><div className="sectionTitle"><div><h2>活跃用户定向分发实验复盘</h2><p>PC 活跃用户 · 每个实验区间统一展示前后两周数据</p></div></div>
    ...
    <section className="pageSection"><div className="sectionTitle"><div><h2>实验前后趋势</h2><p>{formatDate(experiment.beforeStart)}–{formatDate(experiment.end)} · 虚线为周三实验开始</p></div></div><Card className="chartCard"><ActiveExperimentTrendChart items={comparisonItems} markerDate={experiment.start} ariaLabel="PC 活跃用户定向分发实验周期趋势" /></Card></section>
...
function TargetGamePage({ platform }) {
  ...
  const conclusion = isPc ? '8 月完整月进入营收游戏占比 24.22%，较 7 月回落 1.41pp；需在比鸡恢复趋势下继续验证配置效果。' : '8 月完整月进入营收游戏占比 42.44%，较 7 月提升 1.20pp，处于年内高位；本地热门第三轮第 5 位改营收棋牌的结果见实验复盘。';
  return <>
    <header className="pageIntro"><div><h1>{name}分发数据</h1><p>活跃用户进入营收游戏的月度表现</p></div><span>数据更新至 2026/09/06 · 月度完整数据至 8 月</span></header>
...
const NEW_USER_EXPERIMENT_IDS = ['local-hot-v1', 'local-hot-v2', 'local-hot-observation', 'local-hot-waiting-package', 'local-package-prepublish-incident', 'local-package-official-launch', 'local-package-weekly-observation', 'local-package-config-cities', 'total-start-dip-sep4'];
...
function ReviewPage({ experimentIds = EXPERIMENTS.map(item => item.id), title = '实验复盘', subtitle = '按实验区间切换查看 · 7/15 起本地包点击单列展示', showPackageWeeks = false, embedded = false }) {
  ...
  const nextSteps = experimentId === 'total-start-dip-sep4'
    ? [{ title: '优先核查游戏模块开始玩的曝光、排序与承接启动链路。', detail: '9/4 该模块从前 7 日的 20.83% 降至 17.35%（-3.48pp），是当日最强的同步负向模块；核对当天是否有坑位、游戏供给、跳转或启动失败异常。' }, { title: '同步核查搜索游戏链路与流量结构。', detail: '搜索游戏点击较前 7 日 -2.40pp；需补充点击用户到启动用户的承接明细和渠道/版本拆分，才能确认对总启动的真实贡献。' }, { title: '本地包继续按配置推荐点击观察。', detail: '9/4 本地包地区配置 +1.98pp、地区推荐 +1.77pp，当前不支持“本地包走弱导致下跌”的判断。' }]
