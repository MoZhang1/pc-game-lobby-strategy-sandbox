...
import { LATEST_LOCAL_PACKAGE_GEO_DATA, PREVIOUS_LOCAL_PACKAGE_GEO_DATA, CURRENT_WEEK_LOCAL_PACKAGE_GEO_DATA } from './localPackageLatestGeoData';
...
function GlobalEventTable() {
  const all = getSummary('2026-06-20', '2026-09-15');
  const packagePeriod = getSummary('2026-07-15', '2026-09-15');
  const popupPeriod = getSummary(NEW_USER_POPUP.start, '2026-09-15');
...
function GlobalDataPage({ embedded = false }) {
  const [range, setRange] = useState({ start: '2026-08-16', end: '2026-09-15' });
...
function AndroidActiveExperimentReview() {
  const [experimentId, setExperimentId] = useState('active-september15');
...
function PcActiveExperimentReview() {
  const [experimentId, setExperimentId] = useState('pc-september15');
...
function TargetGamePage({ platform }) {
  ...
  const conclusion = isPc ? '9/1–9/15 进入营收游戏占比 24.23%，与 8 月 24.22% 基本持平。最新完整周 24.23%→24.26%（+0.03pp）；联运创角 0.181%→0.174%，上周已完成本地热门优化，当前整体结果基本持平；建议按地区与游戏位复盘承接，本周动作待定。' : '9/1–9/15 进入营收游戏占比 42.90%，较 8 月 42.44% 高 0.46pp。最新完整周 42.71%→43.13%（+0.42pp），建议保留当前配置、观察稳定性，本周新增实验待定。缺少 Banner 点击和对照组，不能单独归因为素材。';
  return <>
    <header className="pageIntro"><div><h1>{name}分发数据</h1><p>活跃用户进入营收游戏的月度表现</p></div><span>数据更新至 2026/09/15 · 9月*仅含1–15日</span></header>
    ...
    <section className="pageSection"><div className="sectionTitle"><div><h2>进入营收游戏趋势</h2><p>按月汇总每日进入营收游戏 UV / 每日用户池 UV（均为人日，非跨日去重）；9月*仅含1–15日，尚非完整月</p></div></div><Card className="chartCard"><TargetGameTrendChart platform={platform} /></Card></section>
...
const NEW_USER_EXPERIMENT_IDS = ['local-package-september15', 'local-hot-v1', 'local-hot-v2', 'local-hot-observation', 'local-hot-waiting-package', 'local-package-prepublish-incident', 'local-package-official-launch', 'local-package-weekly-observation', 'local-package-config-cities', 'local-package-ten-provinces', 'total-start-dip-sep4'];
...
function LocalPackageGeoReview() {
  const geoPeriods = [...LOCAL_PACKAGE_GEO_PERIODS, LATEST_LOCAL_PACKAGE_PERIOD, LAST_LOCAL_PACKAGE_PERIOD, CURRENT_LOCAL_PACKAGE_PERIOD];
  const [periodId, setPeriodId] = useState('2026-09-09_2026-09-15');
  ...
  const comparisonData = isCurrent ? LATEST_LOCAL_PACKAGE_GEO_DATA : PREVIOUS_LOCAL_PACKAGE_GEO_DATA;
  ...
  const insightFor = item => {
    ...
    if (isCurrent || selectedPeriod.id === LAST_LOCAL_PACKAGE_PERIOD.id) {
      ...
      const currentAction = item.pageExposure < 100
      ...
      return { group, issue: '较前周 ' + formatPp(delta) + '；曝光 ' + item.pageExposure, action: isCurrent ? currentAction : delta < 0 ? '复核城市分布、游戏与坑位，下一轮配置待运营确认。' : '正向关联，继续核对稳定性；不自动扩量。' };
    }
  ...
  const renderConfigRate = (item, prior) => isCurrent ? <>{rate(prior?.configuredClicks, prior?.pageExposure)} → {rate(item.configuredClicks, item.pageExposure)}<small>{prior?.pageExposure ? `${prior.configuredClicks} / ${prior.pageExposure}` : '缺少对照'} → {item.configuredClicks} / {item.pageExposure}</small></> : rate(item.configuredClicks, item.pageExposure);
...
function ReviewPage({ experimentIds = EXPERIMENTS.map(item => item.id), title = '实验复盘', subtitle = '按实验区间切换查看 · 7/15 起本地包点击单列展示', showPackageWeeks = false, embedded = false }) {
  ...
  if (experiment.isOngoing) return <>
    {!embedded && <header className="pageIntro"><div><h1>{title}</h1><p>{subtitle}</p></div><span>数据更新至 2026/09/15</span></header>}
  ...
  return <>
    ...
    <section className="experimentBlock"><div className="sectionTitle"><div><h2>{experiment.title}</h2><p>{formatDate(experiment.start)}–{formatDate(experiment.end)} · 对比 {formatDate(experiment.beforeStart)}–{formatDate(experiment.beforeEnd)}</p></div></div>
      ...
      {experimentId === 'local-package-september15' && <DistributionWeeklyDiagnosis />}
...
function AndroidNewUserPage() {
  ...
  return <><header className="pageIntro"><div><h1>安卓新用户分发数据</h1><p>纯新增用户的总启动、模块点击与实验复盘</p></div><span>全局数据至 2026/09/15 · 本地包分城市数据支持 8/17 起滚动观察</span></header>
