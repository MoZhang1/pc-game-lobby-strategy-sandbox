...
function LocalPackageGeoReview() {
  ...
  const insightFor = item => {
    ...
    if (selectedPeriod.id === LAST_LOCAL_PACKAGE_PERIOD.id) {
      const prior = PREVIOUS_LOCAL_PACKAGE_GEO_DATA.provinces.find(row => row.province === item.province);
      ...
      return {group: item.pageExposure < 100 ? '低样本观察' : delta >= 0 ? '表现较好' : '优先优化',issue: '较前周 '+formatPp(delta)+'；曝光 '+item.pageExposure,action:delta < 0 ? '复核城市分布、游戏与坑位，下一轮配置待运营确认。' : '正向关联，继续核对稳定性；不自动扩量。'};
    }
  ...
  return <section className="localPackageGeoReview">
    <section className="pageSection"><div className="sectionTitle"><div><h2>本地包地区分发总览</h2><p>{formatDate(meta.start)}–{formatDate(meta.end)} · 按新增用户口径汇总</p></div></div>
      <Card className="geoToolbar"><div><span>数据时间区间</span><b>{formatDate(meta.start)}–{formatDate(meta.end)}（{meta.days} 天）</b><small>自 8/17 起按滚动周期观察省、市数据</small></div><label className="eventSelect">观察区间<select aria-label="选择本地包地区观察周期" value={periodId} onChange={event => { setPeriodId(event.target.value); setProvince('全部'); setProvinceScope('全部'); }}>{geoPeriods.map(item => <option value={item.id} key={item.id}>{item.label}</option>)}</select></label><label className="eventSelect">省份结论筛选<select value={provinceScope} onChange={event => setProvinceScope(event.target.value)} aria-label="选择省份结论筛选"><option value="全部">全部省份</option><option value="表现较好">表现较好</option><option value="稳定观察">稳定观察</option><option value="待优化">待优化</option><option value="优先优化">优先优化</option><option value="低样本观察">低样本观察</option><option value="无数据">无数据</option></select></label></Card>
      <div className="reviewMetrics geoMetrics"><Metric label="页面曝光UV" value={total.pageExposure.toLocaleString()} helper={`${total.newUsers.toLocaleString()} 新增用户中的本地包页面曝光`} icon={BarChart3} /><Metric label="页面曝光→配置推荐点击" value={rate(total.configuredClicks, total.pageExposure)} helper={`${total.configuredClicks.toLocaleString()} / ${total.pageExposure.toLocaleString()} · 分发配置主指标`} tone="positive" icon={TrendingUp} /><Metric label="地域可归属率" value={rate(classifiedUsers, total.newUsers)} helper={`按城市补回真实省份；${meta.unmappedNewUsers} 新增仍为未知城市`} icon={FileSearch} /></div>
      ...
      <div className="reviewCallout geoConclusion"><b>省级结论</b><p>{selectedPeriod.id === '2026-09-02_2026-09-08' ? <>9/2–9/8 已收齐 7 天。全量页面曝光→配置推荐点击为 <strong>{rate(total.configuredClicks, total.pageExposure)}</strong>，较前周 +0.61pp；10 个配置省份合计 +2.01pp，其余已归属地区 -2.56pp，呈正向关联。山东、内蒙古、安徽、江西提升较明显；江苏、辽宁、山西下降。9/7 当日配置点击率降至 19.71%，需核查当天配置与城市分布。{NEXT_NEW_USER_PLAN}</> : selectedPeriod.id === '2026-08-26_2026-09-01' ? <>首轮只以<strong>页面曝光→配置推荐点击</strong>判断配置效果：全量为<strong>{rate(total.configuredClicks, total.pageExposure)}</strong>。山东较对照期<strong> -17.8pp</strong>，广东<strong> +5.0pp</strong>、安徽<strong> +2.6pp</strong>但样本较小。<strong>9/2 已将运营配置扩展至浙江、湖北、江苏、山东、广东、安徽、内蒙古、江西、辽宁、山西共 10 省</strong>；下一观察周期统一复盘，山东仍优先核对配置游戏、坑位和页面曝光。</> : <>本页只以<strong>页面曝光→配置推荐点击</strong>判断：全量为<strong>{rate(total.configuredClicks, total.pageExposure)}</strong>。在新增≥100的省份中，<strong>{topProvinces.join('、') || '暂无'}</strong>配置表现较好；<strong>{priorityProvinces.join('、') || '暂无'}</strong>需优先调整配置游戏、位置与排序。</>}</p></div>
    </section>

    <section className="pageSection"><div className="sectionTitle"><div><h2>省级配置推荐点击与建议</h2><p>仅以页面曝光→配置推荐点击评估运营配置；低样本不参与优劣排序</p></div></div><Card className="geoTableCard"><div className="geoTable geoInsightTable" role="table" aria-label="本地包省级配置推荐点击与建议"><div className="geoTableHead" role="row"><span>省份</span><span>新增UV</span><span>页面曝光UV</span><span>页面曝光→配置推荐点击</span><span>判断 / 主要差异</span><span>建议动作</span></div>{provinceRows.map(item => { const insight = insightFor(item); return <div className="geoTableRow" role="row" key={item.province}><strong>{item.province}</strong><span data-label="新增UV">{item.newUsers ? item.newUsers.toLocaleString() : '—'}</span><span data-label="页面曝光UV">{item.pageExposure ? item.pageExposure.toLocaleString() : '—'}</span><b data-label="页面曝光→配置推荐点击">{rate(item.configuredClicks, item.pageExposure)}</b><span data-label="判断 / 主要差异"><em className={`geoTag ${insight.group === '表现较好' ? 'good' : insight.group === '优先优化' || insight.group === '待优化' ? 'bad' : ''}`}>{insight.group}</em><small>{insight.issue}</small></span><span data-label="建议动作" className="geoAction">{insight.action}</span></div>})}</div></Card></section>
    <section className="pageSection"><div className="sectionTitle"><div><h2>城市配置推荐点击</h2><p>按城市汇总；以页面曝光UV为分母，低样本只作观察</p></div><label className="eventSelect">查看省份<select value={province} onChange={event => setProvince(event.target.value)} aria-label="选择本地包城市所属省份"><option value="全部">全部城市（{cities.length}）</option>{provinces.filter(item => item.status === '有数据').map(item => <option value={item.province} key={item.province}>{item.province}</option>)}</select></label></div><Card className="geoTableCard"><div className="geoTable cityGeoTable" role="table" aria-label="本地包城市配置推荐点击"><div className="geoTableHead" role="row"><span>省份 / 城市</span><span>归属方式</span><span>新增UV</span><span>页面曝光UV</span><span>页面曝光→配置推荐点击</span></div>{cityRows.map(item => <div className="geoTableRow" role="row" key={`${item.province}-${item.city}`}><strong>{item.province === '未知' ? '待归属' : item.province} · {item.city}</strong><span className={item.mapping === '无法归属' ? 'geoMuted' : ''}>{item.mapping}</span><span data-label="新增UV">{item.newUsers.toLocaleString()}</span><span data-label="页面曝光UV">{item.pageExposure.toLocaleString()}</span><b data-label="页面曝光→配置推荐点击">{rate(item.configuredClicks, item.pageExposure)}</b></div>)}</div></Card></section>
  </section>;
}
