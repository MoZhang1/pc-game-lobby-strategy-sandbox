...
import { LATEST_LOCAL_PACKAGE_GEO_DATA } from './localPackageLatestGeoData';
...
const CURRENT_LOCAL_PACKAGE_PERIOD = {
  id: '2026-09-02_2026-09-06', label: '9/2–9/6（10 省扩展后，5 天）', data: LATEST_LOCAL_PACKAGE_GEO_DATA,
};
...
const DAILY_ROWS = [
  ...
  ['2026-08-26',522,353,14,1,1,0,0,3,71,24,109,54], ['2026-08-27',507,342,12,3,0,0,0,0,65,29,110,46], ['2026-08-28',502,366,12,0,1,0,3,0,74,24,101,48], ['2026-08-29',553,389,20,2,1,0,4,0,62,26,98,53], ['2026-08-30',539,376,15,1,4,0,1,1,72,32,99,47], ['2026-08-31',453,328,19,1,3,1,0,0,57,22,95,54], ['2026-09-01',500,354,18,1,1,0,0,0,57,20,111,49],
  ['2026-09-02',507,353,11,1,1,0,0,1,67,24,123,56], ['2026-09-03',487,328,19,3,1,0,0,1,64,14,108,45], ['2026-09-04',490,328,18,1,2,0,0,3,51,22,85,53], ['2026-09-05',532,383,17,1,3,0,0,0,73,37,101,62], ['2026-09-06',483,335,24,1,3,1,0,2,63,22,99,46],
];
...
const PACKAGE_ROWS = [
  ...
  ['2026-08-26',48,76,3,19], ['2026-08-27',38,71,2,19], ['2026-08-28',50,70,6,28], ['2026-08-29',52,72,8,29], ['2026-08-30',38,93,6,21], ['2026-08-31',35,73,6,22], ['2026-09-01',49,77,3,17],
  ['2026-09-02',41,78,6,22], ['2026-09-03',41,73,7,30], ['2026-09-04',51,84,2,28], ['2026-09-05',42,92,5,26], ['2026-09-06',40,68,3,25],
];
...
const EXPERIMENTS = [
  ...
  {
    id: 'local-package-ten-provinces', observation: true, title: '进行中 · 本地包配置扩展至 10 省', module: '本地包运营配置',
    metricId: '1018905', content: '9/2 扩展至浙江、湖北、江苏、山东、广东、安徽、内蒙古、江西、辽宁、山西。当前收数至 9/6，仅 5 天，完整周期为 9/2–9/8。', start: '2026-09-02', end: '2026-09-06', beforeStart: '2026-08-26', beforeEnd: '2026-09-01',
    analysis: '当前 5 天总启动为 69.11%，较前一周 70.13% 下降 1.03pp；本地包地区配置点击占新增用户为 15.81%，较前一周 14.88% 上升 0.93pp。城市源表页面曝光→配置推荐点击为 25.26%（395 / 1,564），较前一周 23.64% 上升 1.61pp。配置点击与总启动方向不同，不能把总启动下降直接归于本地包。周期尚未结束，待收齐 9/8 数据，按省份与未配置地区对比后判断效果。',
  },
  {
...
const TARGET_GAME_MONTHS = [
  ...
  { label: '7月', pc: { users: 2649323, game: 25.319, union: 0.421, total: 25.632 }, android: { users: 5721197, game: 41.108, union: 0.179, total: 41.237 } },
  { label: '8月', pc: { users: 2589036, game: 24.059, union: 0.220, total: 24.223 }, android: { users: 5651471, game: 42.290, union: 0.205, total: 42.440 } },
  { label: '9月*', pc: { users: 524376, game: 125719 / 524376 * 100, union: 973 / 524376 * 100, total: 126432 / 524376 * 100 }, android: { users: 1076288, game: 457979 / 1076288 * 100, union: 3070 / 1076288 * 100, total: 460198 / 1076288 * 100 } },
];
...
const ACTIVE_EXPERIMENT_DAILY = [
  ...
  ['2026-09-02',180211,76191,589,76622], ['2026-09-03',180812,76891,495,77235], ['2026-09-04',179927,76703,600,77158], ['2026-09-05',177742,75922,393,76192], ['2026-09-06',177446,75827,559,76239],
...
function GlobalEventTable() {
  const all = getSummary('2026-06-20', '2026-09-06');
  const packagePeriod = getSummary('2026-07-15', '2026-09-06');
...
function GlobalDataPage({ embedded = false }) {
  const [range, setRange] = useState({ start: '2026-08-07', end: '2026-09-06' });
  ...
  return <>
    {!embedded && <header className="pageIntro"><div><h1>安卓新用户分发数据</h1><p>总启动与模块点击总览</p></div><span>数据更新至 2026/09/06</span></header>}
    <Card className="filterCard"><div className="filterCopy"><CalendarDays /><div><b>数据日期</b><span>模块数据有效起始 6/20；7/15 起本地包点击单列展示</span></div></div><DateRangeInput label="数据日期" isLabelHidden value={range} onChange={value => value && setRange(value)} min="2026-06-20" max="2026-09-06" numberOfMonths={1} /><Button label="应用筛选" variant="primary" onClick={() => setAppliedRange(range)} /></Card>
...
const ACTIVE_EXPERIMENTS = [
  ...
  { id: 'active-banner-september', title: '进行中 · 9 月首屏 Banner 素材轮换', start: '2026-09-03', end: '2026-09-06', beforeStart: '2026-08-27', beforeEnd: '2026-09-02', baseline: '8/27–9/2', baselineRate: 42.29, baselineHelper: '531,063 / 1,255,652', resultRate: 42.86, resultHelper: '306,824 / 715,927', delta: 0.56, content: '9 月 3 日轮换首屏 Banner 全部素材，观察进入营收游戏占比变化。', conclusion: '当前已收数 4 天，进入营收游戏占比 +0.56pp；周期未满 7 天，仅作阶段性观察，不输出最终实验结论。', next: '继续收齐 9/3 起 7 天数据，再判断素材轮换是否稳定正向。' },
];
...
const PC_ACTIVE_EXPERIMENTS = [
  ...
  { id: 'pc-latest-observation', title: '最新观察期 · 暂无新增配置', start: '2026-09-02', end: '2026-09-06', beforeStart: '2026-08-26', beforeEnd: '2026-09-01', baseline: '8/26–9/1', baselineRate: 24.36, baselineHelper: '148,218 / 608,409', resultRate: 24.14, resultHelper: '104,620 / 433,423', delta: -0.22, content: '本周暂无新增 PC 活跃用户分发配置，继续观察首轮配置后的表现。', conclusion: '当前观察 5 天，变现率 -0.22pp；本周没有新增配置，不将本周波动归因于分发策略。', next: '暂无新增配置动作。已将比鸡、罗松情况通知对应工作室，等待其 Banner 申请；申请未落地前不追加新的 PC 分发实验。', verdict: '观察中' },
];

const PC_ACTIVE_EXPERIMENT_DAILY = [
  ['2026-08-19', 85718, 21171], ['2026-08-20', 86834, 21275], ['2026-08-21', 87123, 21249], ['2026-08-22', 77569, 18715], ['2026-08-23', 76799, 18441], ['2026-08-24', 88125, 21486], ['2026-08-25', 88179, 21723],
  ['2026-08-26', 88363, 21795], ['2026-08-27', 89510, 22063], ['2026-08-28', 90275, 21902], ['2026-08-29', 80026, 19414], ['2026-08-30', 79155, 19252], ['2026-08-31', 90127, 21980], ['2026-09-01', 90953, 21812],
  ['2026-09-02', 92219, 22035], ['2026-09-03', 91452, 22098], ['2026-09-04', 91763, 22183], ['2026-09-05', 79382, 19238], ['2026-09-06', 78607, 19066],
].map(([date, users, total]) => ({ date, label: `${Number(date.slice(5, 7))}/${Number(date.slice(8))}`, users, total: total / users * 100 }));
...
function AndroidActiveExperimentReview() {
  ...
  return <section className="pageSection"><div className="sectionTitle"><div><h2>活跃用户定向分发实验复盘</h2><p>安卓活跃用户 · 按实际配置日展示实验前后数据</p></div></div>
    ...
    <div className="reviewCallout activeUserCallout"><b>本轮结论：{experiment.id === 'active-banner-september' ? '阶段性上升，观察中' : experiment.delta >= 0 ? '正向' : '轻微负向'}</b><p>{experiment.conclusion}</p></div>
    <section className="pageSection"><div className="sectionTitle"><div><h2>实验前后趋势</h2><p>{formatDate(experiment.beforeStart)}–{formatDate(experiment.end)} · 虚线为本轮开始</p></div></div><Card className="chartCard"><ActiveExperimentTrendChart items={comparisonItems} markerDate={experiment.start} /></Card></section>
    <section className="pageSection"><div className="sectionTitle"><div><h2>下一步</h2><p>{experiment.id === 'active-banner-september' ? '当前已收 4 天数据，待收齐 7 天观察期' : experiment.id === 'active-latest-observation' ? '后续动作按实际配置日记录' : '按周三开始，观察 7 天'}</p></div></div><ol className="reviewActions"><li><b>{experiment.next}</b></li></ol></section></section>
  </section>;
}
...
function PcActiveExperimentReview() {
  ...
  return <section className="pageSection"><div className="sectionTitle"><div><h2>活跃用户定向分发实验复盘</h2><p>PC 活跃用户 · 每个实验区间统一展示前后两周数据</p></div></div>
    ...
    <section className="pageSection"><div className="sectionTitle"><div><h2>实验前后趋势</h2><p>{formatDate(experiment.beforeStart)}–{formatDate(experiment.end)} · 虚线为本轮开始</p></div></div><Card className="chartCard"><ActiveExperimentTrendChart items={comparisonItems} markerDate={experiment.start} ariaLabel="PC 活跃用户定向分发实验周期趋势" /></Card></section>
...
function TargetGamePage({ platform }) {
  ...
  const conclusion = isPc ? '9/1–9/6 进入营收游戏占比 24.11%，较 8 月完整月 24.22% 低 0.11pp。9 月尚未结束，仅看阶段走势；本周暂无新增配置，等待比鸡、罗松工作室 Banner 申请。' : '9/1–9/6 进入营收游戏占比 42.76%，较 8 月完整月 42.44% 高 0.32pp。9 月尚未结束；9/3 素材轮换后的 4 天变现率为 42.86%，阶段性上升，完整实验结果待收齐 7 天后判断。';
  return <>
    <header className="pageIntro"><div><h1>{name}分发数据</h1><p>活跃用户进入营收游戏的月度表现</p></div><span>数据更新至 2026/09/06 · 9月*仅含1–6日</span></header>
...
function LocalPackageGeoReview() {
  ...
  const [periodId, setPeriodId] = useState('2026-09-02_2026-09-06');
  ...
  return <section className="localPackageGeoReview">
    <section className="pageSection"><div className="sectionTitle"><div><h2>本地包地区分发总览</h2><p>{formatDate(meta.start)}–{formatDate(meta.end)} · 按新增用户口径汇总</p></div></div>
      ...
      <div className="reviewCallout geoConclusion"><b>省级结论</b><p>{selectedPeriod.id === '2026-09-02_2026-09-06' ? <>10 省扩展后当前已收 <strong>5 天</strong> 数据；全量<strong>页面曝光→配置推荐点击</strong>为 <strong>{rate(total.configuredClicks, total.pageExposure)}</strong>（{total.configuredClicks.toLocaleString()} / {total.pageExposure.toLocaleString()}），未满 7 天，暂不输出配置效果结论。9/4 当日配置推荐点击为 28.28%，高于本期日均水平，<strong>不支持“9/4 总启动下跌由本地包配置走弱导致”的判断</strong>。</> : selectedPeriod.id === '2026-08-26_2026-09-01' ? <>首轮只以<strong>页面曝光→配置推荐点击</strong>判断配置效果：全量为<strong>{rate(total.configuredClicks, total.pageExposure)}</strong>。山东较对照期<strong> -17.8pp</strong>，广东<strong> +5.0pp</strong>、安徽<strong> +2.6pp</strong>但样本较小。<strong>9/2 已将运营配置扩展至浙江、湖北、江苏、山东、广东、安徽、内蒙古、江西、辽宁、山西共 10 省</strong>；下一观察周期统一复盘，山东仍优先核对配置游戏、坑位和页面曝光。</> : <>本页只以<strong>页面曝光→配置推荐点击</strong>判断：全量为<strong>{rate(total.configuredClicks, total.pageExposure)}</strong>。在新增≥100的省份中，<strong>{topProvinces.join('、') || '暂无'}</strong>配置表现较好；<strong>{priorityProvinces.join('、') || '暂无'}</strong>需优先调整配置游戏、位置与排序。</>}</p></div>
    </section>
...
function ReviewPage({ experimentIds = EXPERIMENTS.map(item => item.id), title = '实验复盘', subtitle = '按实验区间切换查看 · 7/15 起本地包点击单列展示', showPackageWeeks = false, embedded = false }) {
  ...
  if (experiment.isOngoing) return <>
    {!embedded && <header className="pageIntro"><div><h1>{title}</h1><p>{subtitle}</p></div><span>数据更新至 2026/09/06</span></header>}
  ...
  const nextSteps = experimentId === 'local-package-ten-provinces' ? [{ title: '收齐 9/2–9/8 完整周期数据。', detail: '按 10 个配置省份分别比较页面曝光→配置推荐点击变化，结合未配置地区作参照；算法推荐点击不计入运营配置效果。' }] : experimentId === 'total-start-dip-sep4'
