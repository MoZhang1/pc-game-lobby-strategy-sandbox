...
function LocalPackageGeoReview() {
  ...
  return <section className="localPackageGeoReview">
    <section className="pageSection"><div className="sectionTitle"><div><h2>本地包地区分发总览</h2><p>{formatDate(meta.start)}–{formatDate(meta.end)} · 按新增用户口径汇总</p></div></div>
      ...
      <div className="reviewCallout geoConclusion"><b>省级结论</b><p>{selectedPeriod.id === '2026-08-26_2026-09-01' ? <>本轮只以<strong>页面曝光→配置推荐点击</strong>判断配置效果：全量为<strong>{rate(total.configuredClicks, total.pageExposure)}</strong>。山东较对照期<strong> -17.8pp</strong>，优先复盘；广东<strong> +5.0pp</strong>、安徽<strong> +2.6pp</strong>但样本较小，继续观察，当前不扩大范围。</> : <>本页只以<strong>页面曝光→配置推荐点击</strong>判断：全量为<strong>{rate(total.configuredClicks, total.pageExposure)}</strong>。在新增≥100的省份中，<strong>{topProvinces.join('、') || '暂无'}</strong>配置表现较好；<strong>{priorityProvinces.join('、') || '暂无'}</strong>需优先调整配置游戏、位置与排序。</>}</p></div>
    </section>
