...
const EXPERIMENTS = [
  ...
  },
  {
    ...
    analysis: '完整周总启动 70.13% → 68.34%（-1.79pp），本地包总点击占新增 24.16% → 24.19%，基本持平；主要下降位于总启动减本地包总点击后的剩余部分（45.97% → 44.16%，-1.82pp），该差额仅作口径衔接定位，不是模块去重归因。运营配置主指标：页面曝光→配置推荐点击 23.64% → 24.26%（+0.61pp）；10 省合计 22.29% → 24.30%（+2.01pp），其余已归属地区 26.81% → 24.25%（-2.56pp）。呈正向关联，但非随机对照，不能证明因果。江苏、辽宁、山西配置点击率下降，需重点复核。9/4 与 9/7 低谷机制不同，详见本期诊断。',
  },
  {
    id: 'total-start-dip-sep4', observation: true, title: '异常诊断 · 9/4 总启动低点', module: '总启动 / 游戏模块开始玩',
    metricId: '60103909', content: '9/4 总启动降至 66.94%，以 8/28–9/3 的 7 天数据为参照，定位当日同步变化模块。', start: '2026-09-04', end: '2026-09-04', beforeStart: '2026-08-28', beforeEnd: '2026-09-03',
  },
];
...
function GlobalDataPage({ embedded = false }) {
  ...
  return <>
    ...
    <Card className="filterCard"><div className="filterCopy"><CalendarDays /><div><b>数据日期</b><span>模块数据有效起始 6/20；7/15 起本地包点击单列展示</span></div></div><DateRangeInput label="数据日期" isLabelHidden value={range} onChange={value => value && setRange(value)} min="2026-06-20" max="2026-09-08" numberOfMonths={1} /><Button label="应用筛选" variant="primary" onClick={() => setAppliedRange(range)} /></Card>
    ...
    <div className="reviewCallout"><b>最新完整周：9/2–9/8</b><p>总启动 70.13% → 68.34%（-1.79pp）。9/7 为最新低点 65.32%；9/4 主要是非本地包同步走弱，9/7 则本地包配置点击也下降。完整诊断与10省实验结果见“实验复盘”，本轮不预设新实验。</p></div>
...
function AndroidActiveExperimentReview() {
  ...
  return <section className="pageSection"><div className="sectionTitle"><div><h2>活跃用户定向分发实验复盘</h2><p>安卓活跃用户 · 按实际配置日展示实验前后数据</p></div></div>
    ...
    <section className="pageSection"><div className="sectionTitle"><div><h2>实验前后趋势</h2><p>{formatDate(experiment.beforeStart)}–{formatDate(experiment.end)} · 虚线为实际配置日或观察周分界</p></div></div><Card className="chartCard"><ActiveExperimentTrendChart items={comparisonItems} markerDate={experiment.markerDate || experiment.start} /></Card></section>
    <section className="pageSection"><div className="sectionTitle"><div><h2>下一步</h2><p>{experiment.id === 'active-banner-september' ? '周报已收齐；下一轮方案待确认' : experiment.id === 'active-latest-observation' ? '后续动作按实际配置日记录' : '按周三开始，观察 7 天'}</p></div></div><ol className="reviewActions"><li><b>{experiment.next}</b></li></ol></section></section>
  </section>;
}
...
function PcActiveExperimentReview() {
  ...
  return <section className="pageSection"><div className="sectionTitle"><div><h2>活跃用户定向分发实验复盘</h2><p>PC 活跃用户 · 每个实验区间统一展示前后两周数据</p></div></div>
    ...
    <section className="pageSection"><div className="sectionTitle"><div><h2>下一步</h2><p>等待工作室 Banner 申请后再确定下一轮配置</p></div></div><ol className="reviewActions"><li><b>{experiment.next}</b></li></ol></section></section>
  </section>;
}
...
function TargetGamePage({ platform }) {
  ...
  const conclusion = isPc ? '9/1–9/8 变现率 24.20%，较 8 月 24.22% 低 0.02pp。最新完整周 24.36% → 24.23%（-0.13pp），首轮配置尚未表现出持续提升；等待工作室 Banner 申请及后续配置确认。' : '9/1–9/8 变现率 42.70%，较 8 月 42.44% 高 0.26pp。9/2–9/8 完整周提升 0.52pp；9/3 素材轮换后同星期六天对比提升 0.49pp。缺少活跃用户 Banner 点击明细，只能确认整体改善，不能单独归因为素材。';
  return <>
    ...
    <section className="pageSection"><div className="sectionTitle"><div><h2>进入营收游戏趋势</h2><p>按月汇总：进入营收游戏 UV / 用户池 UV；9月*仅含1–6日，尚非完整月</p></div></div><Card className="chartCard"><TargetGameTrendChart platform={platform} /></Card></section>
...
function ReviewPage({ experimentIds = EXPERIMENTS.map(item => item.id), title = '实验复盘', subtitle = '按实验区间切换查看 · 7/15 起本地包点击单列展示', showPackageWeeks = false, embedded = false }) {
  ...
  const candidate = driverItems.filter(item => (experimentId === 'total-start-dip-sep4' || item.id !== experiment.metricId) && Math.sign(item.delta) === Math.sign(totalDelta)).sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta))[0];
  ...
  const nextSteps = experimentId === 'local-package-ten-provinces' ? [{ title: '本轮复盘后由运营确认下一轮配置。', detail: '先核查江苏、辽宁、山西及 9/7 配置点击回落；不自动回退、扩量或安排新实验。算法推荐点击不计入运营配置效果。' }] : experimentId === 'total-start-dip-sep4'
    ? [{ title: '优先核查游戏模块开始玩的曝光、排序与承接启动链路。', detail: '9/4 该模块从前 7 日的 20.76% 降至 17.35%（-3.41pp），是当日最强的同步负向模块；核对当天是否有坑位、游戏供给、跳转或启动失败异常。' }, { title: '同步核查搜索游戏链路与流量结构。', detail: '搜索游戏点击较前 7 日 -2.38pp；需补充点击用户到启动用户的承接明细和渠道/版本拆分，才能确认对总启动的真实贡献。' }, { title: '本地包继续按配置推荐点击观察。', detail: '9/4 本地包地区配置 +2.01pp、地区推荐 +1.77pp，当前不支持“本地包走弱导致下跌”的判断。' }]
