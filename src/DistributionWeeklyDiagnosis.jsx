...
export default function DistributionWeeklyDiagnosis({geoOnly=false}){
  return <section className="channelCompare">
    {!geoOnly && <>
      ...
      <p><b>整周：总启动 70.64% → 68.90%（-1.74pp）。</b>新增 3,576 → 3,440，启动 2,526 → 2,370。启动人数减少 6.18%，快于新增人数减少 3.80%。</p>
      <p>本地包总点击占新增 24.16% → 24.19%，基本持平；按总启动减本地包总点击计算的剩余部分为 46.48% → 44.71%（-1.77pp）。这是基于两表口径衔接的差额，不是把所有非本地包模块 UV 相加，也不能证明单个模块的因果贡献。</p>
      <p><b>9/4：本地包没有走弱，游戏模块开始玩与搜索下降。</b>总启动 67.96%；较前 7 日，游戏模块开始玩 -3.21pp、搜索 -2.38pp，而本地包配置点击占新增 +2.01pp。9/3、9/4 启动人数为 330 → 333，新增分母为 487 → 490，总启动占比小幅回升 0.20pp。</p>
      <p><b>9/7：本地包与其他入口同时走弱。</b>总启动 294 / 444 = 66.22%，较 8/31–9/6 加权基线 70.28% 低 4.06pp。本地包总点击占新增 24.80% → 21.40%（-3.40pp），剩余部分 45.48% → 44.82%（-0.66pp）。本地包配置点击占新增 15.79% → 12.16%（-3.63pp），算法推荐 8.66% → 6.31%（-2.36pp）；游戏模块开始玩 -1.58pp，搜索反而 +2.13pp。各点击人群重叠，以上模块变化不可相加。</p>
      <p><b>9/8：有所恢复，仍未回到基线。</b>总启动 67.81%，本地包配置点击 / 页面曝光恢复至 23.25%，高于 9/7 的 19.71%。目前更像分日波动叠加非本地包转化走低，不能归为持续的本地包配置失效。</p>
