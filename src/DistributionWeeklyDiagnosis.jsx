...
export default function DistributionWeeklyDiagnosis({ geoOnly = false, data = currentData }) {
  ...
  return <section className="channelCompare weeklyDiagnosis">
    ...
    <div className="channelPanel"><h3>{isLatest ? '本周待办 · 9/23–9/29（待定）' : '当时计划 · 9/16–9/22'}</h3><p>{isLatest ? NEXT_NEW_USER_PLAN : PREVIOUS_NEW_USER_PLAN}</p><p>地区源表没有分城市的总启动、分游戏或坑位数据；先依据页面到达与配置点击定位省市，再核对具体游戏和位置。</p></div>
